...
class Text
{
    public static function seoUrl(string $value): string
	{
        $table = array(
            'A' => 'A',
            'Ⓐ' => 'A',
    ...
    /**
     * Vrati regex string pre dany string - konvertuje na male pismena
     * @param string $string
     * @return string regex
     */
    public static function regexFromString(string $string,$patternPrefix=null,$patternPostfix=null): Regex
    {
        $map = [
            'a' => 'aáäą',
            'á' => 'aáäą',
        ...
        $array  = self::str_split_unicode(strtolower($string), 1);
        $string = '';

        foreach ($array as $char) {
            $string .= (isSet($map[$char])) ? '[' . $map[$char] . ']' : $char;
        }
		if($patternPrefix){
			$string=$patternPrefix.$string;
		}
		if($patternPostfix){
			$string=$string.$patternPostfix;
		}
        return new Regex($string, 'iu');
    }
